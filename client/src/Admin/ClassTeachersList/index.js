...
export default function ClassTeachersList({
  setClassTeacherId,
  changeSetActive,
  createStudent,
}) {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_FETCH_URL}/getclassTeacherList`,
        options
      );
      ...
      console.log(data);
      const updatedList = data.classTeachers.map((emp, index) => ({
        ...emp,
  ...
  const handleClick = () => {
    changeSetActive("AddClassTeacher");
    // navigate("/AddClassTeacher");
  };
          ...
          {employees.length === 0 ? (
            <>
              <div className="projects-not-found-container">
                <h1 className="not-found-text">No Class Teachers Found </h1>
                <img
                  src={notfound}
                  alt="No Bills Found"
