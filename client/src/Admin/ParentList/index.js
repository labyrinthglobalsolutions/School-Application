...
import DeleteEmploeeModalPage from "../TeachersList/verifyAdmin";
...
export default function ParentsList({
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_FETCH_URL}/getparents`,
        options
      );
      ...
      const updatedList = data.parents.map((emp, index) => ({
        ...emp,
  ...
  const deleteEmployeeId = async (id) => {
    ...
    if (confirmDeletion) {
      ...
      try {
        const response = await fetch(
          `${process.env.REACT_APP_FETCH_URL}/deleteParent/${id}`,
          options
        );
        const data = await response.json();
        if (!response.ok) {
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    ...
    try {
      ...
      const api = `${process.env.REACT_APP_FETCH_URL}/updateStatusOfParent/${id}`;
          ...
          {employees.length === 0 ? (
            <>
              <div className="projects-not-found-container">
                ...
                />
                <button className="add-project-button" onClick={addParent}>
                  Add Parent
                </button>
              </div>
            </>
