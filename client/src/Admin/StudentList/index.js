...
export default function StudentList({ getStudentId, createStudent }) {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_FETCH_URL}/getstudents`,
        options
      );
      ...
      const updatedList = data.students.map((emp, index) => ({
        ...emp,
  ...
  const deleteEmployeeId = async (id) => {
    ...
    if (confirmDeletion) {
      ...
      try {
        const response = await fetch(
          `${process.env.REACT_APP_FETCH_URL}/deleteStudent/${id}`,
          options
        );
        const data = await response.json();
        if (!response.ok) {
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    ...
    try {
      ...
      const api = `${process.env.REACT_APP_FETCH_URL}/updatestatusofstudent/${id}`;
          ...
          {employees.length === 0 ? (
            <>
              <div className="projects-not-found-container">
                ...
                />
                <button className="add-project-button" onClick={addEmployee}>
                  CREATE STUDENT
                </button>
              </div>
            </>
